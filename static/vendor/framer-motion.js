...
import { useContext as useContext5, useId, useEffect as useEffect3, useCallback as useCallback2 } from "react";
function usePresence() {
  ...
  const safeToRemove = useCallback2(() => onExitComplete && onExitComplete(id4), [id4, onExitComplete]);
...
function MotionConfig({ children, isValidProp, ...config }) {
  ...
  const context = useMemo7(() => config, [JSON.stringify(config.transition), config.transformPagePoint, config.reducedMotion]);
...
import { forwardRef as forwardRef2, useRef as useRef8, useEffect as useEffect6 } from "react";
...
function ReorderGroup({ children, as = "ul", axis = "y", onReorder, values, ...props }, externalRef) {
  ...
  return jsx9(Component3, { ...props, ref: externalRef, ignoreStrict: true, children: jsx9(ReorderContext.Provider, { value: context, children }) });
}
...
function ReorderItem({ children, style = {}, value, as = "li", onDrag, layout: layout2 = true, ...props }, externalRef) {
  ...
  return jsx10(Component3, { drag: axis, ...props, dragSnapToOrigin: true, style: { ...style, x: point2.x, y: point2.y, zIndex }, layout: layout2, onDrag: (event, gesturePoint) => {
...
import { useContext as useContext13, useRef as useRef9, useInsertionEffect as useInsertionEffect3 } from "react";
function toNumber(v) {
  if (typeof v === "number")
    return v;
  return parseFloat(v);
...
function useSpring(source, config = {}) {
  ...
  const latestValue = useRef9(value.get());
  const latestSetter = useRef9(() => {
  });
...
function useScroll({ container, target, layoutEffect = true, ...options } = {}) {
  ...
  const useLifecycleEffect = layoutEffect ? useIsomorphicLayoutEffect : useEffect8;
  useLifecycleEffect(() => {
...
import { useRef as useRef10, useContext as useContext14, useEffect as useEffect9 } from "react";
...
import { useRef as useRef11, useState as useState6, useCallback as useCallback4 } from "react";
function useCycle(...items) {
  const index = useRef11(0);
  const [item, setItem] = useState6(items[index.current]);
  const runCycle = useCallback4(
    (next) => {
...
import { useState as useState8, useEffect as useEffect14 } from "react";
var createObject = () => ({});
...
var AnimateSharedLayout = ({ children }) => {
  ...
  return jsx11(LayoutGroup, { id: useConstant(() => `asl-${id3++}`), children });
};
